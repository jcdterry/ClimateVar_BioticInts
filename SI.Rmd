...
This is a dynamically generated document using R Markdown. Source code (.rmd) for this document is available https://github.com/jcdterry/ClimateVar_BioticInts, allowing complete replication. Code to generate the diagrams in figures 2 and 3 is visible in the .rmd source file, but not included here.
...
There must be as many alternative models as partitions, and then the contribution of each process can be found through straightforward differences between the models (Table S3). $\Delta_\ast$ is an 'interaction term' term that captures how the combined effect of the underlying non-linearities differs from a direct summation of their individual effects. As such, it doesn't have a direct ecological interpretation. Here we just fit one overall interaction term, but in principle (as we have three underlying non-linearities: $\Delta_\lambda,\Delta_\alpha,\Delta_R$) we could have built additional models to identify each pairwise interaction term too. However, in most cases this is unlikely to bring additional ecological insight and there are likely to be limited additional gains for ever finer partitions given the requirements for parameterisation.
...
E_Partition<- function(Params){
  ...
  ## Variable E draws:
  E_mat_full  <-  mvrnorm(n_reps,   mu = c(0,0),  Sigma = W_SIGMA_corr   )  
  E_mat_uncorr<-  mvrnorm(n_reps,   mu = c(0,0),  Sigma = W_SIGMA_uncorr   ) 
...
ggsave( 'MS_Figures/Figure4_graphs.png', Fig4, height = 7, width = 10)
...
In Figure 6 of the main text,  we present some examples of how different sources of variability can interact in their effect on species response to climate change. For this analysis we extend the basic model detailed above to include climate change, immigration, demographic stochastic and Allee effects. 
...
During period of climate change, starting at $t = t_{cc}$, an underlying environment term $\gamma$ increases smoothly from 0 to 1 sat the end of the trial ($t_{max}$). Each species response to the underlying environment is controlled by $\beta_s$ via $\omega$:
...
$$I_{t+1}  =    \delta I_t    + \dfrac{\lambda_I \omega_{I,t} I_t}{1 +a_{II}I_t +a_{IR}\omega_{R,t}R_t}+ d_t $$
...
$$I_{t+1}  =    \delta I_t   +  \dfrac{\lambda_I \omega_{I,t} I_t^2  }{1  +(z +yI_t)I_t +a_{IR}\omega_{R,t}R_t}+ d_t$$
...
ggsave('MS_Figures/Fig6LineDiag.png', Fig6LineDiag, height = 3, width = 3)
...
ggsave('MS_Figures/Fig6_GR.png', Fig6_GR, height = 3, width = 3)
```
...
|$\eta$ | 0 |0|0| 0 |                         
|$\rho$     |  X |0| 0 | 0.9|                        
|$a_{RR}$   |0.02| 0.02 | 0.03 | 0.05|                         
|$a_{II}$   |0.03|0.05|0.06 |0.08|
|$y$, $z$   | -  |0.1,0.2| -| -|
|$a_{RI}$   |0.03|0.03|0.04 |0.03|
|$a_{IR}$   |0.04|0.04| 0.03|0.04|           
...
ggsave(filename = 'MS_Figures/Fig6_simple.png',  Fig6_simple, width = 6, height = 6)
...
Performances%>%
  ...
  geom_histogram()+
  theme_classic()+
  geom_vline(data = LineValuesMeans, aes(xintercept = value,linetype = key))+
...
# Fig2
ggsave('MS_Figures/Figure2.png', height = 3, width = 7)
```
